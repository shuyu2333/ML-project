			...
			for key in feed_dicts[0]:
				if isinstance(feed_dicts[0][key], np.ndarray):
					...
					if any([tmp_list[0] != l for l in tmp_list]):
						# use Python object dtype for ragged arrays
						stack_val = np.array([d[key] for d in feed_dicts], dtype=object)
					else:
						stack_val = np.array([d[key] for d in feed_dicts])
				else:
					stack_val = np.array([d[key] for d in feed_dicts])
				# robustly detect object dtype arrays (ragged / inconsistent length)
				if getattr(stack_val.dtype, 'kind', '') == 'O':  # inconsistent length (e.g., history)
